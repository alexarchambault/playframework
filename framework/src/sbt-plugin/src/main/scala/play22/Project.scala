...
object Project extends Plugin with PlayExceptions with play22.Keys with PlayReloader with PlayCommands
    with PlayRun with play22.Settings with PlayPositionMapper with PlaySourceGenerators {
  ...
  Option(System.getProperty("play.version")).map {
    case badVersion if badVersion != play22.core.PlayVersion.current => {
      println(
        Colors.red("""
          |This project uses Play %s!
          |Update the Play sbt-plugin version to %s (usually in project/plugins.sbt)
        """.stripMargin.format(play22.core.PlayVersion.current, badVersion))
      )
    }
    case _ =>
  }
